...
- 所有新增备忘、待解决问题、讨论结论和后续待办统一记录在 [`docs/planning/研究问题与待办总备忘录.md`](docs/planning/研究问题与待办总备忘录.md)，不再另建按日期、主题或会话分散的备忘录、问题清单或待办副本。
- 更新已有事项时沿用其编号，保留来源、当前状态、下一步和关闭依据；已合并的旧专题备忘录和旧问题清单删除工作区副本，来源通过总备忘录中的迁移记录与Git历史追溯，不再另存备忘副本。
- 总备忘录不替代编码表、正式协议、研究决策、冻结manifest或原始编码记录。实质规则变更仍须同步相应规范；平台中的原始疑问留在原处，只将需要研究讨论的去敏摘要汇总到总备忘录。
...
- 代码必须有结构完备且与实现同步的注释和 docstring，注释主体使用简体中文；标识符、协议字段和专有名词可保留英文。模块、公开类与函数应说明职责、输入输出、不变量、边界条件和失败语义，复杂分支应解释“为什么”，避免只逐行复述代码。
- 模块必须按单一职责解耦。配置解析、持久化、领域计算、调度与状态机、命令行入口分别组织，通过明确的数据结构和接口协作；禁止循环依赖、跨层读取内部状态，以及把业务规则堆积在脚本入口中。
- 正式实验应记录配置、随机种子、数据 manifest、代码版本和评估结果。
- 数据清洗的科研方案与工程方案共享稳定契约名和状态，不显示协议版本号；标签、抽样、算法、阈值原则或决策状态变化时，必须在同一 commit/PR 中同步更新两份文档。配置、模型、算法、CSV、manifest 与运行仍保留内部 ID、哈希和谱系。
