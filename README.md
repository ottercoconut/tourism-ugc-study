...
> 2026-09-11执行状态：用户在销毁旧尝试后，重新授权在AutoDL从头进行全量推理。
> 当前为`REMOTE_RESTART_PREPARING`；保留现有SSH登录方式，不修改防火墙或认证。
> 每批500条回传本机并校验后再释放服务器该批数据；源快照与冻结模型不变，
> 尚无本轮完整候选交付。旧MPS/Runpod尝试不恢复、不混入新结果。
...
历史13,858条已完成冻结模型下的三段式路由交付。2026-09-11新的19,255条研究输入已启动全量清洗；候选经人工审查后才发布和冻结最终研究数据集。当前状态、结果口径和模型决策的详细说明见 [数据清洗当前状态与执行索引](docs/protocols/数据清洗当前状态与执行索引.md)。
