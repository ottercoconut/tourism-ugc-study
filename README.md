...
> 2026-09-11执行状态：AutoDL新全量推理、回传及整库验收已完成，当前为
> `CANDIDATES_READY_AWAITING_HUMAN_REVIEW`。19,255条候选为keep 10,914条、
> exclude 5,305条、manual_review 3,036条；正式keep表仍为0，等待人工审查。
> 全部36批预测、向量与回执已在本机归档，远端批次数据已清理；源快照、
> 冻结模型及现有SSH模式不变，旧MPS/Runpod推理产物不恢复、不混入新结果。
...
历史13,858条结果不适用于新人口。2026-09-11新的19,255条研究输入已完成全量清洗候选验收，私有候选库为`data/processed/research-cleaning-20260911-autodl-r2/cleaning-candidates.sqlite`；候选经人工审查后才发布和冻结最终研究数据集。当前状态、结果口径和模型决策的详细说明见 [数据清洗当前状态与执行索引](docs/protocols/数据清洗当前状态与执行索引.md)。
