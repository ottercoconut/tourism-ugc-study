...
数据清洗 v3.2 是纯文本/帖子流程：派生 schema v33 覆盖只读帖子快照、全量确定性结构门、单轴青岛旅游相关性审核、平台比例概率抽样、分量级多样化定向抽样、线性相关性基线、帖子最终决定、分析去重和不可变发布。清洗阶段不读取、下载、审核、筛选或发布媒体文件；视觉数据在后续视觉研究阶段按独立协议处理。工程测试通过不代表正式数据已经清洗或发布，正式人工证据、质量审计和 formal `accept-release` 仍须按协议完成。
