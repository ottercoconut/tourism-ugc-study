...
文本清洗工程框架已冻结，当前研究状态为 `FRAMEWORK_FROZEN / REFERENCE_DEDUP_FINALIZED / ACCEPTANCE_AND_AUDIT_FROZEN / TEST_PENDING`。通用 `cleaning.yaml` 仍保留未授权时的安全默认值；正式 Qwen 双阈值、锁定测试判读和双尾审计由独立内容寻址配置绑定。唯一最终700条及其 leakage build 已封存，论文内容编码和视觉模型仍不得创建带有猜测参数的默认配置或空子目录。
