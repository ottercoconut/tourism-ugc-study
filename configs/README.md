...
当前稳定入口为 `configs/cleaning.yaml`。它只冻结全局字符 TF-IDF、线性 SVM、分组折外 Sigmoid 校准、全局时间留出和三段式策略接口；平台最低名额、平台负例门和平台审计配额均不进入配置。`T_keep`、`T_exclude`、保留集审计门、测试指标门、自动覆盖率门和恢复门当前全部为 `UNSET`。源库、派生库、参考 CSV、参考 manifest、目标批次与 `run_id` 仍由命令行显式传入，不写入共享配置。
