...
下列文件已完成对编码表v3.6.1的语义对齐。文本轨现为`CALIBRATION_READY / NOT_FROZEN`：编码员单表和负责人后处理模板已经落实，可以进行共同校准；剩余冻结门关闭前不能进行独立盲试标或正式标注。
...
- [`planning/Label Studio工程难度简报.md`](planning/Label%20Studio工程难度简报.md)：面向工程负责人的文本标注平台工作量、难度、阶段门与延期边界说明；不授权实施。

## 待同步的当前文档

下列文件仍含编码表v3.4.2时期的正文，状态为`NEEDS_UPDATE`。它们的上位版本和本轮直接相关记录已对齐v3.6.1，但尚未完整承接v3.4.3—v3.6.1的全部字段与模型变化，修复前不得作为当前执行依据。

- [`data-dictionary/编码维度主观性评定报告.md`](data-dictionary/编码维度主观性评定报告.md)：主观性与重叠风险复核。
...
- [`../manuscript/论文草稿.md`](../manuscript/论文草稿.md)：论文Markdown草稿；其字段与方法表述尚未完整统一为v3.6.1口径。
