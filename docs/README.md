...
下列文件已完成对编码表v3.10.0的语义对齐。文本轨现为`CALIBRATION_READY / NOT_FROZEN`：固定Excel人工编码表、编码员长表和负责人后处理模板已经落实，可以进行共同校准；V2新增活动字段必须纳入本轮校准且旧`rs_r_act`不得自动迁移。视觉轨仍须先冻结研究人口、V11/V12边界与V16专门规则。剩余冻结门关闭前不能进行正式标注或模型发布。
...
- [`decisions/2026-08-21-V2休闲游憩与节事文体事件拆分决策.md`](decisions/2026-08-21-V2休闲游憩与节事文体事件拆分决策.md)：V2活动资源拆分、边界及重编码要求。
