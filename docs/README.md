...
下列文件已完成对编码表v3.6.0的语义对齐。文本轨现为`CALIBRATION_READY / NOT_FROZEN`：可以进行共同校准，但剩余冻结门关闭前不能进行独立盲试标或正式标注。
...
下列文件仍含编码表v3.4.2时期的正文，状态为`NEEDS_UPDATE`。它们的上位版本和本轮直接相关记录已对齐v3.6.0，但尚未完整承接v3.4.3—v3.6.0的全部字段与模型变化，修复前不得作为当前执行依据。
...
- [`../manuscript/论文草稿.md`](../manuscript/论文草稿.md)：论文Markdown草稿；其字段与方法表述尚未完整统一为v3.6.0口径。
