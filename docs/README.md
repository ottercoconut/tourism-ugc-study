...
下列文件已完成对编码表v3.11.0的语义对齐。现行12个大类按作者/帖子、文本、图像观察单位连续编号为V0—V11，字段名、值域和观察单位未改变。文本轨现为`CALIBRATION_READY / NOT_FROZEN`：固定Excel人工编码表、编码员长表和负责人后处理模板已经落实，可以进行共同校准；V4新增活动字段必须纳入本轮校准且旧`rs_r_act`不得自动迁移。视觉轨仍须先冻结研究人口、V7/V8边界与V11专门规则。剩余冻结门关闭前不能进行正式标注或模型发布。
