...
| 图片人工工作 | 30 张共同试标；全部拟排除候选代表双标；另取 50 张边界样本双标；保留集每轮 200 张概率审计 |
...
8. 决策应用后，从保留内容图中每轮稳定抽取 200 张概率审计；零残余技术噪声时单侧 95% Wilson 上限约为 1.33%。发现问题则冻结新决定版本并重做一轮，最多三轮；仍不通过时保持 `review`。
...
- 候选代表复核、拟排除代表双标、每轮 200 张保留集概率审计、仲裁和技术噪声标签；
- SHA 精确簇的有限标签传播、`image_decisions` 和图片分析视图；pHash 自动传播不属于当前方案；
...
| `schema.py` | schema v15、迁移、外键、不可变与封存 trigger | 不计算图片特征 |
...
### 13.10 Issue #10 修正后的最小人工工作流

Issue #10 不训练图片分类器，也不要求研究者重新识别头像或页面图。`relation_role`、文件状态、SHA/pHash 和候选信号均直接消费 Issue #9 的封存结果；新的人工证据只回答一个问题：`content` 候选是否属于图片技术噪声。
...
实现时应把下列对象分别持久化，具体表名可在迁移设计中确定，但身份与边界不能合并：

1. 候选导出运行及稳定成员 manifest：绑定 `candidate_build_id`、标签手册、配置、随机种子与代码版本。
2. 原始人工标签：保存对象、标注槽位、标注者匿名 ID、手册版本、标签、理由代码与时间；只追加。
3. 仲裁：引用两条原始标签，不覆盖原记录；仲裁者不得是任一原标注者。
4. 精确 SHA 传播运行：绑定已确认代表、成员 manifest 与输出摘要；只传播 `technical_noise_label`。
5. 保留集概率审计轮次：绑定决定版本、平台配额、纳入概率、权重、预期/实际成员 manifest 和 Wilson 区间。

#### 13.10.3 人工量与停止门

1. 先共同试标 30 张真实候选，记录平均单张耗时并冻结 `image-noise-v1.0` 边界示例；这 30 张只用于手册校准，不作为概率估计。
2. 稳定导出技术信号、SHA 簇与 pHash 对的代表项。单人可先判明显 `valid_content`；所有拟排除代表必须进入第二个独立槽位，分歧或任一 `uncertain` 交第三人仲裁。
3. 另冻结 50 张覆盖候选与保留边界的盲双标集，报告原始一致率、Cohen's κ 和分标签分歧。原始一致率低于 0.80 时修订手册并追加最多 50 张；κ 仅在至少出现两个标签时计算，单类别导致不可估不单独触发扩样。该集合经过定向富集，只评估手册可重复性，不估计总体噪声率。
4. 决策应用后，从 `analysis_images_eligible` 候选保留集中按平台稳定抽取 200 张概率审计；小于 200 时全查。每个平台先分配 `min(30, N_keep,p)` 张，其余按保留量分配并保存权重。
5. 首轮零残余技术噪声时，单侧 95% Wilson 上限约 1.33%，满足 2% 控制线。发现任何残余技术噪声时修正规则或人工决定，冻结新决定版本并重新抽取 200 张非重叠审计；最多三轮，仍不通过则运行保持 `review`。
6. 每新增约 20,000 张内容图时，从 true-new 内容图抽取 100 张漂移审计并复核全部新拟排除代表；平台或采集器结构改变时立即触发。
...
| 5. 图片噪声复核 | 候选代表导出、拟排除代表双标/仲裁、精确 SHA 有限传播和每轮 200 张保留集概率审计；pHash 只组织复核 | 每个排除与传播均有人工证据；pHash 无传播；保留集审计达到科研方案控制线 | 2–4 人日＋数据依赖的人工复核时间 |
| 6. 决策合并与正式发布 | 帖子/图片最终决策、文本/图片就绪视图、去重视图、质量报告、运行验收和回滚 | 只有 `accepted` 运行可供论文分析；计数、哈希、版本和理由链完整 | 2–4 人日 |

当前输入契约、派生调度、确定性文本处理、文本人工工作流、相关性模型接口和阶段 4 图片技术候选框架已实现。仍须执行正式文本标注与仲裁；模型 smoke 不形成旅游相关性决定。阶段 4 目前只有合成夹具框架验收，真实图片到位后仍须 dry run；阶段 5 只对拟排除候选和质量审计样本形成经人工确认的图片噪声证据，不对头像/page、文件阻塞或全部内容图重复人工标注；阶段 6 负责人工优先的正式发布，不反向修改前面的历史记录。

一名熟悉 Python、SQLite 和 scikit-learn 的工程成员，完整稳健实现预计约 16–28 人日，另加数据依赖的人工复核时间；阶段 0–1 的第一可用里程碑约 4–7 人日。图片人工工时须在 30 张共同试标后按候选代表数估算，不再预先假设固定 600 张或 300 对。计算成本较低，主要风险是状态、版本、人工标签和源对象变化的可追溯性。
