...
| 当前数据库状态 | 3,044 条三城混合的阶段性数据，其中青岛 2,274 条；尚不是正式清洗输入 |
| 当前图片关系 | 27,704 条，其中 `content` 26,776、`author_avatar` 915、`page` 13 |
...
下表来自 2026-07-28 对正式库的只读核验，只是工程输入审计。正式运行必须从冻结快照重新生成，不得把这些数字硬编码进规则。

| 只读观察 | 当前结果 | 工程处理 |
| --- | --- | --- |
| 当前库尚未达到正式输入状态 | `web_posts` 3,044 条；青岛 2,274、济南 454、烟台 315、城市缺失 1 | 这是阶段性审视记录。后续采集与数据集定稿工作将另行形成只含青岛的正式输入；清洗模块不处理、过滤或标记当前其余 770 条 |
| 当前青岛子集的平台分布 | B站 652、微博 561、小红书 401、抖音 386、知乎 274 | 只用于预估抽样工作量；正式配额必须从后续冻结输入重新计算 |
| 青岛来源与时间 | 2,274 条均为 `mediacrawler_search`；发布时间 2013-06-20 至 2026-07-21，采集时间 2026-07-13 至 2026-07-21 | manifest 同时保存发布时间与采集时间分布；时间测试按发布时间建立 |
| 平台字段形态 | 青岛正文和发布时间无空值；微博 561 条标题均为空；每条青岛记录均有 `content` 图片关系 | 可用性规则分平台；微博空标题不触发 `invalid` |
| 青岛图片关系 | 19,622 条，其中 `content` 18,947、`author_avatar` 675；声明图片数与 `content` 关系数按平台一致 | 头像直接离开内容图视图；图片总量必须按 `image_role` 分开报告 |
| 图片文件信息尚缺 | 青岛图片关系的 `local_path`、`sha256`、尺寸和 MIME 均为空 | 文件 SHA、pHash、尺寸和背景图规则在本地图片 manifest 可用前标记为 `blocked_by_manifest`，不能伪造结果或在线补抓 |
| URL 与文本重复候选 | 青岛内容图有 534 个重复 URL 组、涉及 1,505 条关系；规范化文本有 13 个精确重复组、涉及 27 条、理论多出 14 条 | URL 复用和文本相同只生成簇；源关系不删除，近重复仍需另行评估 |
...
**职责边界：**本工程不把当前混合库转换为青岛数据集。正式清洗只接受已经完成城市范围定稿的输入；若入口检查发现非青岛或城市缺失记录，运行以 `input_rejected` 结束并返回上游处理，不生成逐条清洗决策。
...
2. 以只读查询验证 `city_name` 非空且唯一值为 `青岛市`。不满足时写入运行级 `input_rejected` 和汇总原因后停止；不得在本模块中筛选记录或生成逐条范围标签。
3. 使用 SQLite backup API 从合格正式库创建一致性临时快照。
4. 记录源路径、源库与快照 SHA-256、文件大小、表行数、输入契约结果和 UTC/Asia-Shanghai 时间。
5. 验证正式库构建前后 SHA-256 不变。
6. 运行期间只访问快照，不继续读取变化中的正式库。
...
`analysis_posts_eligible` 必须显式过滤 `cleaning_decision='keep'`；`analysis_images_eligible` 只能关联这些帖子。城市唯一性已在入口契约检查中完成，此处只作防御性断言，绝不据此过滤行。所有视图必须通过明确的 `run_id` 构建，禁止依赖“最新一行”或隐式时间排序。
...
- 输入契约接受仅含青岛的夹具；遇到济南、烟台、城市缺失或混合城市时整次运行返回 `input_rejected`，且不生成逐条清洗决策。
...
- 输入契约和防御性断言均确认 `COUNT(DISTINCT city_name)=1` 且唯一值为 `青岛市`；不满足时没有任何合格视图产出。
...
| --- | --- | --- | ---: |
| 0. 输入契约与骨架 | 固定只读源库接口、配置加载、日志脱敏、迁移框架和脱敏测试夹具 | 混合城市输入能安全拒绝；源库写入测试必然失败 | 1–2 人日 |
