...
执行基线为 CPython 3.13.5、NumPy 2.5.1、regex 2026.7.19、scikit-learn 1.9.0、SciPy 1.18.0 和 joblib 1.5.3；PyYAML 保持 `>=6.0,<7`。`configs/cleaning-v2.4.yaml` 当前固定 `algorithm_versions.derived_schema=14`，确定性文本运行时另由 `text_runtime` 哈希锁定。所有命令必须通过项目 `.venv/bin/python` 执行。
...
- schema v14 冻结运行/快照身份、保持状态字段合法流转，并继续执行同 build 组合外键、build/manifest/指纹版本/行身份一致性、`building→finalized` 候选构建和显式阻塞恢复；
...
### 13.7 schema v14 与不可变性

fresh database 直接迁移至 v14；已有 v10-v13 派生库保留合法数据并幂等升级，不修改正式采集库。`cleaning_runs` 的协议、配置摘要、随机种子、代码版本、环境、创建时间和运行类型为冻结身份；`source_snapshot_id` 只允许从 NULL 绑定到同运行已经插入的快照一次。状态机仍可更新 `status/reason_code/started_at_utc/finished_at_utc/updated_at_utc`。`source_snapshots` 的源路径与摘要、快照路径与摘要、源 schema/计数/对象 manifest、输入契约、代码/环境和创建身份整行禁止更新或删除。
...
- schema v14 fresh/幂等、v10-v13 合法数据升级、运行配置/快照路径与摘要/快照重绑直接 SQL 拒绝、状态字段合法更新，以及跨 manifest 成员、同 build 组合外键和封存后写入拒绝。
...
截至候选成员归属、历史快照延迟导入和去敏失败契约回归补齐后，`tests/cleaning` 收集 110 项；本轮仓库完整测试结果为 `137 passed`（仅有既有 NumPy/joblib 弃用警告）。后续验收仍以目标提交上的完整命令结果为准，不能只引用图片子集。
