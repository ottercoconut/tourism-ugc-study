...
> 方案版本：`2.4`
> 同步日期：`2026-08-01`
> 当前工程基线：派生 schema `v25`；Issue #4–#11 的非人工工程目标已验收
> 自动验收：CPython `3.13.5`，`332 passed`、`8 warnings`，`compileall`、`git diff --check` 与科研 HTML 解析通过
...
本方案同时记录已实现基础设施和正式执行边界。当前已具备派生库、增量调度、确定性文本规范化、重复候选、文本抽样/追加式标注、人工确认近重复的泄漏分组、离线相关性基线与复核候选，以及图片角色、本地 manifest、只读文件指纹、重复候选、最小人工复核、SHA 精确传播和两层保留集审计框架。Issue #11 在当前 schema v25 中实现了帖子候选/最终决定、平台比例文本保留集审计、分析去重、四类 release-scoped 输出、去敏质量报告、不可变本地包和显式发布验收边界；`scripts/build_research_dataset.py` 仍只覆盖既有字段规范化和部分派生逻辑，不属于新版清洗流水线。正式人工标签与正式模型运行尚未发生，真实图片尚未落盘验收，正式文本/图片审计和 formal `accept-release` 也未执行；因此当前只能称工程接口已实现，不能称清洗数据已经发布。

> **文本双轴契约已落实：**schema v23 完成双轴迁移，当前 schema v25 沿用这一契约并增加最终决定/发布对象及正式验收不变量。CSV 导出/导入、仲裁、一致性、帖子决定与文本保留集审计只保存并验收结构可用性与旅游相关性；`structure_label=invalid` 必须对应 `tourism_label=not_applicable`。商业属性不属于清洗表单、门槛、模型目标、帖子决定或发布输出。旧三轴派生库可迁移，但旧手册版本和旧一致性结果不能冒充 `text-cleaning-v1.0` 的正式证据。
...
截至 2026-08-01，Issue #4–#11 中可由代码、schema、CLI、合成夹具、只读源库检查和自动化测试证明的工程目标均已完成。工程 Issue 可以关闭，但这不等于正式数据已经清洗、正式模型已经训练或论文数据已经发布。所有必须由研究者在真实文本、真实图片和正式冻结快照上完成的工作，统一转由 [Issue #12](https://github.com/ottercoconut/tourism-ugc-study/issues/12) 跟踪；在 #12 的证据门全部通过前，不存在论文可用的 formal accepted `release_id`。

| Issue | 工程交付 | 可自动核实的完成证据 | 工程状态 |
| --- | --- | --- | --- |
| #5 | 输入契约、只读快照、配置与迁移骨架 | 输入拒绝、`mode=ro/query_only`、动态 manifest、迁移幂等与源库摘要不变测试 | 已验收 |
| #6 | 对象版本、冻结批次、状态机与增量恢复 | 连续快照、指纹分轴、幂等发现、WAL/单写者、阻塞/失败恢复测试 | 已验收 |
| #7 | 确定性文本、结构候选与重复候选 | 黄金文本、固定运行时、精确簇/近似候选重建、断网和不可传播测试 | 已验收 |
| #8 | 文本清洗双轴标注、泄漏分组、线性基线与复核队列 | schema v23→v25 契约、盲标/仲裁、切分、smoke 上限、模型越权负例 | 已验收；正式人工转 #12 |
| #9 | 图片角色、受控 manifest、文件指纹与候选 | 程序生成图片、路径/解码失败、固定 SHA/pHash、断网和源文件不变测试 | 已验收；正式图片转 #12 |
| #10 | 图片双标、仲裁、决定、SHA 传播与两层审计 | pilot/boundary 门、直接 SQL 负例、census/Wilson、平台补充与最多三轮测试 | 已验收；真实复核转 #12 |
| #11 | 帖子 final、分析去重、四类发布、不可变包与显式接受 | 合成端到端、数量守恒、artifact 篡改、只读验收、attestation/guard 与事务回滚测试 | 已验收；formal 发布转 #12 |

本轮完整自动验收使用项目 `.venv/bin/python` 执行，共收集并通过 332 个测试；8 条警告均来自 joblib 在 NumPy 2.5 下的既有弃用提示，不影响通过状态。另以临时派生库对正式采集 SQLite 执行一次只读快照冒烟，入口返回成功，源文件 SHA-256 与大小在运行前后保持一致；临时派生库随测试目录销毁，没有回写正式库。该冒烟只证明输入与只读边界可运行，不产生正式清洗结果。
...
| --- | --- |
| 清洗方案版本 | `2.4` |
| 文本标签手册 | `text-cleaning-v1.0`；schema v25 沿用双轴契约 |
...
本章是 Issue #8 对应实现的工程手册，记录该子系统在 schema v23 完成双轴修正时的真实代码行为，而不是未来接口设想；仓库当前派生 schema 已升级为 v25，新增内容见 Issue #11 实现记录。原始实现基线为 `data-cleaning` 分支提交 `e869c8a`，schema v23 在不改变抽样与模型算法的前提下修正清洗双轴契约；后续如果修改抽样口径、标签手册、泄漏边、切分算法、阈值目标、运行身份或封存规则，必须同时更新本章、配置、迁移和相应测试。
...
执行基线为 CPython 3.13.5、NumPy 2.5.1、regex 2026.7.19、scikit-learn 1.9.0、SciPy 1.18.0 和 joblib 1.5.3；PyYAML 保持 `>=6.0,<7`。`configs/cleaning-v2.4.yaml` 当前固定 `algorithm_versions.derived_schema=25`：v15 是 Issue #9 的图片指纹固定边界，v16–v22 追加 Issue #10 的人工证据、决定、传播、审计、派生评估、审计人口/抽样可信封存及协议 seed 绑定约束，v23 删除文本清洗商业列并加入结构无效/旅游不适用约束，v24 新增 Issue #11 的帖子决定、文本保留集审计、分析去重和正式发布对象，v25 补齐图片决定完整性与正式验收防绕过迁移；确定性文本运行时另由 `text_runtime` 哈希锁定。所有命令必须通过项目 `.venv/bin/python` 执行。
...
以下字段和约束记录 schema v23 建立、并由当前 v25 继续沿用的 `text-cleaning-v1.0` 清洗专用表单工程口径。
...
- `post_decisions`、文本保留集审计、分析去重代表项和四类分析输出的工程接口已由 schema v25 / Issue #11 实现，但尚未用正式人工证据构建 accepted release；
...
| `image_decision_repository.py` | 为每个 content fingerprint 构建自身决定，并只传播经确认的 SHA 技术噪声排除 | `image_decision_builds/decisions/evidence_links`、`image_sha_propagation_runs/members` |
...
所有父对象均以 `building` 插入，核对实际成员计数和 manifest 后单向转为 `finalized`；封存后子表不能追加、更新或删除。人工标签、导入、仲裁、协议评估和审计标签全都只追加。复核运行绑定 `candidate_build_id/guide/config/seed/code/member_manifest`；annotation 必须与 import、run、guide 同源；决定构建为 candidate build 的完整 fingerprint 集逐项保存自己的决定，缺少或额外出现任一 fingerprint 都不能封存。SHA 代表按候选证据决定，非代表没有自身人工证据时独立记为无标签 `default_keep_no_candidate`，不得继承代表的 `single_valid_content`、默认 keep 或人工证据。只有代表经双标同意或仲裁确认的四类排除技术噪声，才以 `image_sha_propagation_*` 覆盖精确簇成员。审计人口逐项读取成员自己的 `keep/review` 决定，再扣除来源决定、四类标签、精确簇、代表、成员集合与 manifest 均完整匹配的 finalized 排除传播成员；任何 keep、默认决定或不完整传播都不能扩大或缩小人口。轮次继续在 `image_keep_audit_population_members` 冻结身份、平台和人口次序；seed 必须沿 `decision_build→candidate_build→cleaning_run` 等于运行基种子加“轮号−1”，SQLite 与应用层再以该 seed/SHA-256 次序重建 primary、平台补充、概率、权重、三份 manifest 和 census/Wilson 身份。只有 `seal_status/integrity_status` 均 finalized 的轮才能导出任务、导入标注、评估或成为续轮证据。一致性与审计评估自身也遵循 `building→逐条原始 annotation 链接→重算核对→finalized`，评估表中的状态、计数或比例不再能作为自己的来源证据。

schema 的实际演进为：v16 建立基础人工证据、决定、传播和审计对象；v17 把 annotation manifest 纳入一致性评估身份，使 `incomplete→complete` 能追加留存；v18 曾将审计人口从代表展开为同一 SHA 簇关系；v19 增加 `content` 候选硬边界、import/run/guide 同源、仲裁必要性与安全原因码、规范决定证据链接、pilot/boundary 正式门禁、仅双标/仲裁技术排除可 SHA 传播，以及“上一轮失败＋决定 manifest 已变化”的审计重试状态机；v20 为两类评估增加原始标注链接和单向封存触发器，在 SQLite 重算 pair/事件计数、原始一致率、κ、census/Wilson、状态与理由，同时在应用层再次从底层证据逐字段重建；v21 增加追加式人口快照、实际人口/期望成员视图、SQLite SHA-256 rank/manifest 函数和轮完整性状态，逐项证明人口、平台、跨轮排除、两层选择、概率与权重；v22 在父行写入和封存时都沿真实运行谱系校验 seed，并在升级时把 seed 不匹配的 v21 finalized 轮降为 `untrusted_legacy`；v24 增加最终决定、分析去重和发布对象；v25 修正为“candidate 完整 fingerprint 集各有自身决定＋仅完整 confirmed exclude 传播”，并增加正式发布 attestation 与连接 guard。v24→v25 是有条件的数据保留迁移：升级审计先回查来源决定与传播运行属于同一 decision build/candidate build，来源 fingerprint 是对应精确簇代表，且来源决定由双标一致或仲裁确认四类 exclude 之一、决定/证据/传播来源标签完全一致；再从真实精确簇重算传播人数、双向完整成员集与规范 member manifest，不相信自报的 expected count 或 finalized 状态。任一旧传播不完整时整次迁移以稳定错误拒绝且不改写原库。若旧库已有 accepted run/release 却不存在对应 v25 attestation，迁移也安全阻断并保留历史原样；须在新的 v25 派生库按当前不变量重新构建并显式接受，不能自动继承旧信任。只有不存在这些不可信状态和非法传播时，迁移才保留既有父子行和审计历史、创建新证明对象并替换相关视图/触发器/约束。v19 既有评估和 v20 及更早审计轮仍只作历史审计。
...
本章记录 schema v25 的真实工程边界。Issue #11 不重新训练模型、不增加清洗标签，也不把上游候选改名为最终结果；它把 Issue #8 的文本人工/模型证据和 Issue #10 的图片技术决定，组合成可审计、不可变、可显式接受的分析发布。当前实现只通过脱敏/合成 fixture 的工程 smoke；正式人工文本证据、正式模型、真实图片、真实文本/图片保留集审计与真实快照验收均未完成，所以不存在可供论文使用的 accepted `release_id`。
...
2. 显式人工仲裁 `invalid + not_applicable` 或 `usable + unrelated`：`exclude`；`usable + related`：`keep`；
...
final 构建不重新读取一套“最新证据”，而是显式引用一个 finalized candidate 构建和该 candidate keep 人口的 passed `text_keep_audit_evaluation_id`。仓储复制并重算 candidate 决定身份，只有此前低风险待审计项在通过评估后转成正式低风险 keep 理由；人工确认的 `invalid + not_applicable` 保持人工证据来源并排除，其他人工/确定性决定也保持原谱系。每条 final 决定哈希都绑定实际采用的文本保留集评估 ID，不能以同一帖子身份替换另一次评估。这样审计人口在 final 之前冻结，发布又只接受 final，结构上消除循环依赖。
...
分析去重构建显式绑定 final 帖子决定构建和 finalized 文本候选构建。程序先在完整 final 帖子人口上读取选定关系；只有同一确认关系闭包中相互冲突的显式人工双轴标签才写入 `review_members` 并阻断该去重构建，普通排除、未人工标注或单纯处于 review 的成员不冒充这种簇内冲突。所有 final `review` 无论是否位于重复簇，都会由发布全局接受门阻断。冲突门通过后，正式去重人口才投影为 final `keep` 全集。允许的边只有：

- `exact`：确定性文本精确关系；
- `human_duplicate`：明确列出的近重复人工仲裁，且决定必须为 `duplicate`。

近重复候选 pair、候选连通分量、训练 leakage component、未明确列出的仲裁和模型相似度都拒绝。程序从允许边重新计算连通分量，单例也进入一个簇；每簇按版本化稳定策略选择且只选择一个代表，成员/代表/边/manifest 封存于 `text_dedup_*`。相互冲突的显式人工双轴标签只形成复核证据，不用代表覆盖；处理后须重建 final 决定与去重构建。去重只决定 `analysis_posts_deduplicated` 展示哪一条，不修改 `post_decisions`、不物理删除、不传播结构/旅游标签，也不影响未去重 `analysis_posts_eligible`。
...
| `content + own technical keep + parent post keep` | 该 fingerprint 自身决定为 keep 且未被合法 SHA 排除传播覆盖时进入 `analysis_images_eligible` |
...
这里没有 `visual_relevance`、商业属性或内容形式轴。路线图、推荐计划图、旅游攻略长图和游戏截图都不能因主题/形式由发布层清除；只要其自身 Issue #10 技术决定为 keep 且父帖 keep，就与普通照片相同。pHash 仍只组织候选；SHA 精确关系也只能传播经确认的四类技术噪声排除，单人 `valid_content`、默认 keep 和 uncertain 都不能在发布时扩散。

### 14.5 四类输出、质量报告与数量守恒

`analysis_release_builds` 显式绑定 `release_id/run_id/source_snapshot_id`、formal/smoke、final 帖子构建、分析去重构建、文本评估、图片决定、图片评估，以及协议、schema、配置、代码和请求 manifest。`output_root` 只是本机 artifact 的落盘位置，不进入科研 request manifest、发布身份或幂等键；同一科研请求不会因为换磁盘目录而变成另一份数据。四类成员表均带 `release_id/run_id` 和源版本：
...
质量报告只保存去敏身份、计数、哈希和稳定理由码，至少包括帖子决定/理由、去重边/簇/代表、图片角色、content 自身技术决定、合法排除传播、父帖投影理由和四类输出计数；谱系报告列出显式上游构建/评估身份。封存时 schema 重新证明：final keep 数等于帖子合格成员数；每个去重簇恰有一个合格代表；父帖 keep、content 自身决定 keep 且未被合法排除传播覆盖的关系与图片合格成员全集相等；page 实际关系与证据成员全集相等；两类报告和五类 DB manifest 齐全。任何缺行、多行、跨 release/run、旧源版本或计数不守恒都不能 finalized。
...
`accept-release` 只接受 formal finalized，并重新检查：运行/快照/协议/配置/代码一致；冻结 snapshot 以 SQLite URI `mode=ro` 重开，实际读得 `PRAGMA query_only=1` 与 `quick_check=ok`，且核对前后 SHA-256 和文件大小不变；final 帖子和图片决定无 `review`；文本、图片审计均 finalized/passed/formal 且谱系匹配；没有必需 task 处于 pending/running/failed/blocked；四类成员、计数、DB manifest、质量/谱系报告和磁盘 artifact 全部重算一致。验收服务把快照摘要/大小、只读方式、query_only、完整性结果和 artifact manifest 哈希组成不可变 attestation；只有全部复验完成后，才在内部为当前连接安装一次性、阶段绑定且与上述证据字段绑定的 guard。单一事务严格依次写入 attestation、把 run 转为 `accepted`、再把 release 转为 `accepted`；每一步只能消费对应 guard 阶段，事务结束或任一步失败都把 guard 恢复为恒假。外部仓储/API 不暴露 guard 武装入口，普通连接默认恒假，调用者不能提供任意哈希绕过文件验收；任一步失败整事务回滚，不能出现只接受一端。
...
| `analysis_release_repository.py` | 显式 build/verify/status、成员/报告与 artifact 持久化编排 | 不负责接受状态；只使用显式 ID，不把 output root 纳入科研 request manifest |
| `release_acceptance.py` | 复验冻结快照与 artifact、生成证明并独占 accepted 事务 | 内部管理一次性阶段 guard；非 SQLite、非 query_only、哈希/大小或 artifact 变化均失败 |
| `schema.py` v25 | 表、CHECK、外键、封存/不可变/accepted 防绕过 trigger 与 v24 数据保留迁移 | 直接 SQL 也不能跳过谱系、审计、数量和状态门 |
| `scripts/cleaning_release.py` | `build/verify/status/accept-release` 参数解析和去敏 JSON 回执 | 必须显式 run/release；无 latest，不承载规则 |

v25 trigger 覆盖 candidate/final 谱系、证据类型回查、审计人口/样本/评估封存、分析去重允许边与唯一代表、candidate 完整 fingerprint 决定集及 confirmed exclude 传播、四类成员角色/决定/父帖约束、release 成员和报告完整性、父子不可变、状态单向转换、smoke 禁止 accepted，以及不可变 acceptance attestation 与一次性分阶段 connection guard。SHA 传播封存不采用父行自报值，而是先核验来源决定与当前 decision build/candidate build、精确簇代表一致，且双标同意或仲裁确认四类 exclude、来源标签全链一致；再重算真实精确簇人数、双向成员集完全相等和规范 member manifest。升级审计使用同一算法，发现不完整旧传播即拒绝整个迁移。应用层仍在每次封存/验收时独立重算，不能把数据库 CHECK 当作唯一验证，也不能通过直接 SQL、伪 attestation 或任意哈希武装绕过磁盘与只读快照门。v24→v25 的迁移测试必须分别证明：可信旧库的历史行保留、迁移幂等和外键完整；非法传播稳定阻断且原库不变；缺少 v25 attestation 的 accepted run/release 稳定阻断且不继承信任。

### 14.8 合成验证边界与研究者待办

工程验收使用脱敏 SQLite、小规模合成文本和程序生成图片/成员摘要验证候选→审计→final→去重→四类输出与不可变包；合成发布只能是 smoke/finalized，不能写入 formal accepted。验收结构测试可以证明连接级 guard、attestation 和事务顺序会拒绝直接 SQL，但不能把测试授权包装成真实研究发布。测试覆盖阈值边界、平台比例抽样、同人口重抽、非法双轴、模型越权、候选/component 泄漏、标签传播、父帖投影、角色混淆、数量不守恒、敏感字段、包篡改、smoke 接受、直接 SQL 和事务回滚。它证明接口和防绕过规则，不证明正式数据污染率、模型表现、图片噪声率、人工耗时或最终样本规模。最终全量回归以提交验收记录为准。

下列事项不属于工程 Issue #4–#11 的关闭条件，现统一由 Issue #12 跟踪。研究者仍需提供或执行：
...
阶段 0–6 已按独立提交、独立测试与阶段审查完成工程交付。下图现在表示已实现依赖关系，不再是待开发路线；正式真实数据执行沿同一顺序进行，但由 Issue #12 集中跟踪。
...
| 建设阶段 | 范围与主要交付 | 自动验收依据 | 当前状态 |
| --- | --- | --- | --- |
| 0. 输入契约与骨架 | 只读源库、配置加载、日志脱敏、迁移框架和脱敏夹具 | 输入范围拒绝、源库写入失败、摘要不变、迁移幂等 | 工程已验收 |
| 1. 派生数据库与增量调度 | schema、inventory、源版本、冻结批次、状态机和断点恢复 | 连续快照分类、稳定任务集合、WAL/单写者、显式恢复 | 工程已验收 |
| 2. 确定性文本清洗 | 规范化、结构候选、精确重复和近重复候选 | 黄金样例、相同哈希、可重建簇、断网执行 | 工程已验收 |
| 3. 文本人工标注与相关性 | 抽样导出、追加式标签/仲裁、泄漏组、TF-IDF＋SVM、阈值分流和审计持久化 | 合成盲标/仲裁、无泄漏切分、smoke 上限与模型越权负例 | 工程已验收；正式人工在 #12 |
| 4. 图片角色与文件指纹 | 角色分流、本地 manifest、解码元数据、SHA-256、pHash、候选和阻塞恢复 | 程序生成图片、路径边界、固定指纹、断网与源文件不变 | 工程已验收；正式 manifest 在 #12 |
| 5. 图片噪声复核 | 候选复核、动态双标、仲裁、SHA 有限传播与两层保留集审计 | 合成 pilot/boundary、证据门、直接 SQL 负例、census/Wilson 和跨轮状态机 | 工程已验收；真实复核在 #12 |
| 6. 决策合并与正式发布 | candidate→文本审计→final、分析去重、四类输出、质量报告、不可变包和 `accept-release` | 合成端到端、数量守恒、artifact 复验、attestation/guard 与事务回滚 | 工程已验收；formal 发布在 #12 |

当前输入契约、派生调度、确定性文本处理、文本人工工作流、相关性模型接口、图片技术候选、图片复核/决定/审计和最终发布工程框架均已实现并通过非人工验收。仍须执行正式文本标注、仲裁、正式模型和文本保留集审计；模型 smoke 不形成旅游相关性决定。图片链目前只有合成夹具框架验收，真实图片到位后仍须 manifest dry run、真实人工复核和概率审计；图片复核不对头像/page、文件阻塞或全部内容图做候选式人工标注。合成 build/finalized 也不等于发布，只有 #12 的全部真实门通过并显式 `accept-release` 后才产生论文可用 release；它不反向修改前面的历史记录。

历史开发估算为 16–28 人日，现不再用该估算判断完成度。剩余工作量全部依赖正式冻结数据和研究者判断：文本按本方案固定抽样量执行；图片人工工时须在 30 张共同试标后按“候选代表首标＋拟排除/不确定的第二槽＋边界双标＋仲裁＋主样本及平台补充审计”估算，不预先假设固定 600 张、固定每轮 200 张总量或 300 对 pHash。
...
科研 HTML 还应使用 Python 标准库 `html.parser.HTMLParser` 完整解析一次。正式源库只读冒烟必须使用临时派生 SQLite 调用 `scripts/cleaning_snapshot_source.py`，并在命令前后独立计算源文件 SHA-256 和字节数；测试产物不得写入仓库。以上检查的 2026-08-01 基线为 `332 passed, 8 warnings`，其含义仅是工程契约通过。

### 15.2 正式执行的唯一剩余入口

研究者先完成 #12 的文本、图片和保留集证据，再由工程人员显式选择上游构建 ID 执行发布。`scripts/cleaning_release.py build` 只能生成 `building → finalized` 发布；随后必须分别执行 `verify` 和 `accept-release`。`accept-release` 只接受 formal、全部真实证据通过、源快照与 artifact 复验一致的发布。任何缺证据、`review`、`blocked`、版本错配、包篡改或直接 SQL 绕过都会失败，不存在 latest 或自动 accepted 路径。
