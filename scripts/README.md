...
> **数据清洗状态**：`FRAMEWORK_FROZEN / THRESHOLD_PENDING / IMPLEMENTATION_PENDING`。当前已提供稳定配置、700 条参考证据只读校验和 500/200 样本迁移 manifest 生成；正式训练、阈值和发布仍未启用。
...
稳定配置入口为 `configs/cleaning.yaml`，所有阈值与门均为 `UNSET`；因此即使旧代码可以执行某些步骤，也不得据此产生正式自动保留或自动排除。`cleaning_validate_reference.py` 只读打开派生库，并拒绝已经导入 `text_post_annotations` 的参考标签。
