...
def _copy_content(source: sqlite3.Connection, target: sqlite3.Connection) -> dict[str, Any]:
    ...
    selected = "SELECT id FROM web_posts WHERE " + FILTER
    tables = [
        ("schema_migrations", "1", "version"),
        ("source_platforms", "platform_key IN (SELECT platform_key FROM web_posts WHERE " + FILTER + ")", "platform_key"),
        ("ctf_captures", "id IN (SELECT source_capture_id FROM web_posts WHERE " + FILTER + ")", "id"),
        ("web_posts", FILTER, "id"),
